		...
		<div className="w-full mx-auto py-2 px-6 md:px-10 selection:bg-teal-500/30">
			...
			<div className="relative border-l-2 border-slate-200/60 dark:border-slate-800 pl-8 ml-3 space-y-5">
				{data.map((item: SkillCategory) => (
					// Use item.category as the unique key
					<div key={item.category} className="relative group">
						...
						<div className="space-y-2 transform transition-all duration-300 group-hover:translate-x-1">
							<h5 className="text-sm font-bold font-mono uppercase text-muted-foreground group-hover:text-teal transition-colors duration-200">
								{item.category}
							</h5>
							...
							<div className="flex flex-wrap gap-2 pl-3">
								{item.items.map((skill: string) => (
									<span
										key={skill}
										className="cursor-default rounded-lg px-3 py-1 text-[11px] font-medium transition-all duration-200 ease-in-out bg-muted text-muted-foreground border border-border/70 hover:-translate-y-0.5 hover:border-teal/40 hover:text-teal hover:bg-teal-pale"
									>
										{skill}
									</span>
								))}
