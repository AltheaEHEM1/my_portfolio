  ...
  return (
    <div className="w-full py-8 space-y-6">
      {education.map((edu, index) => (
        <div key={index} className="relative flex justify-center lg:justify-start lg:pl-8 group">
          {/* The Dot */}
          ...
          <div className="relative w-full max-w-lg border border-black/20 dark:border-border rounded-xl p-5 bg-transparent font-mono transition-all duration-300 hover:border-teal/50 hover:shadow-xl"> 
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-3">
              <h3 className="text-base font-bold text-foreground uppercase tracking-wide">{edu.school}</h3>
              <span className="mt-2 sm:mt-0 text-[11px] font-bold bg-teal/10 text-teal px-4 py-1 rounded-full whitespace-nowrap border border-teal/20">{edu.period}</span>
            </div>
            <p className="text-sm font-semibold text-muted-foreground">{edu.degree}</p>
